...
# return a dictionary with key = code and value = country + name, includes all the indices
async def EOD_get_description():
	url = "https://eodhistoricaldata.com/api/exchange-symbol-list/INDX?api_token=" + EOD_KEY + "&fmt=json"
	html = await generic_get(url)
	description_dict = dict()
	for item in json.loads(html):
		description_dict[item['Code'] + ".INDX"] = item['Country'] + " " + item['Name']
